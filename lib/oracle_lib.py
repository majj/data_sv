...
class OracleDB:
    ...
    def __init__(self, db_info=None):
        ...
        self.conn = OracleDB.__getConn(self.db_info)
    ...
    def __getConn(db_info):
        ...
        ### dsn = db_info['dsn']
        ### mypool = cx_Oracle.SessionPool(user=user,password=password,dsn=dsn)
        ### conn = cx_Oracle.connect(pool = mypool)

        ### return conn

        if OracleDB.__pool is None:
            __pool = PooledDB(
                cx_Oracle,
                user=db_info["username"],
                password=db_info["password"],
            ...
            log.debug(db_info["dsn"] + " connected")
    ...
    def callproc(self, name, params):
        ...
        for item in params:
            if item == 'number':
                proc_params.append(self.cur.var(cx_Oracle.NUMBER)) # o_
            elif item == 'string':
                proc_params.append(self.cur.var(cx_Oracle.STRING)) # o_
            else:
                proc_params.append(item)
                
        return self.cursor.callproc(name, proc_params)
        """
        try:
    ...
    def backup(self, table, userid, remark, cond_dict):
        ...
        if rows[0]["cnt"] == 0:  # 注意oracle列名大写
            self.execute(
                "create table log_%(table)s as  select * from %(table)s where 1>2 "
                % locals())
            self.execute(
                " alter table log_%(table)s add(backup_userid varchar2(40),\
    ...
    ## 得到存储过程返回的cursor数据
    def get_ref_rows(self, ref, is_dict=True):
        """ get ref cursor data (after called procudure) """
        ...
        if is_dict:
            dict_rows = []
            dict_keys = [r[0].lower() for r in ref.description]
            for row in rows:
                dict_rows.append(dict(zip(dict_keys, row)))
            rows = dict_rows
        return rows
